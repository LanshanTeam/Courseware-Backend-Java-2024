...
时间和知识有限，大家目前对这一块的要求就是知道会用常用的SQL语句操作MySQL，知道MySQL、Redis是拿来干嘛的
...
### 缓存三兄弟、IO模型：
缓存穿透：
		请求的数据在数据库和缓存中都不存在，这样的请求永远打到数据库上
		解决方案：
				• 把这个数据缓存null值到redis中，但这样如果后续新增了这个数据，就查不出来，直接被redis拦下来了，所以要设置个有效期
				• 布隆过滤器，快速判断数据库中存在哪些数据，存在才放进去，不存在直接返回，但这样可能误判，而且删除数据后也会把请求放进数据库
				
缓存击穿：
		某个热点key缓存突然失效，导致大量请求瞬间打到数据库
		解决方案：
				• 互斥锁，当缓存失效时，通过锁只放一个线程去数据库中查数据并更新到缓存中，但这样其它查询线程都会阻塞
				• 逻辑过期，设置逻辑过期时间，逻辑过期后，一个线程去查最新数据，其余线程直接返回逻辑过期后的数据，可能存在不一致问题
				• 不设过期时间，热点数据直接不设过期时间，更新了就重建，可能产生数据不一致问题
				
缓存雪崩：
		大量key同时失效，或redis宕机，导致大量请求打到数据库
		解决方案：
				• 不同的key的TTL添加随机值
				• Redis部署集群保证可用性
				• 业务添加多级缓存
